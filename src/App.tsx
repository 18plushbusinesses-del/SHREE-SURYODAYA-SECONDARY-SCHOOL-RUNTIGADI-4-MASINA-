...
import { SeeCornerSection } from './components/SeeCornerSection';
...
import { Notice, NoticeCategory, TargetAudience, SeeVideo } from './types';
...
export default function App() {
  ...
  const [seeVideos, setSeeVideos] = useState<SeeVideo[]>(mockSeeVideos);
  ...
  // Handle URL Deep-Linking for single notice detail view (?noticeId=...) or tab (?tab=see)
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-slate-100/70 text-slate-900 flex flex-col font-sans">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 py-6 space-y-8">
        ...
        {/* SEE Examination & Video Learning Hub View */}
        {(activeSection === 'see' || activeSection === 'notices') && (
          <SeeCornerSection 
            videos={seeVideos} 
            lang={lang}
            onOpenPdf={(url, title) => {
      ...
      {adminOpen && (
        <AdminDashboardModal
          ...
          notices={notices}
          seeVideos={seeVideos}
          onAddSeeVideo={(newVid) => setSeeVideos(prev => [newVid, ...prev])}
          onDeleteSeeVideo={(vidId) => setSeeVideos(prev => prev.filter(v => v.id !== vidId))}
